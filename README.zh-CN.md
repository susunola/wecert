...
它返回 `202 Accepted`，收敛在后台跑；结果靠 `GET /hook/status` 轮询。定时器和事件触发可能在同一时刻落到同一张证书上，所以占位是**同步**做的，第二个调用方拿到的是 `skipped`，而不是下一个重复订单 —— 那会直接撞上"该 identifier 集合每 7 天 5 张"的限额。token 是必填的，且至少 16 个字符；这个端点会真实签发，所以请把它当凭证对待。详见[配置参考 → `webhook`](README.reference.zh-CN.md#webhook)。
...
> `-prune-certs` 按别名前缀 `wecert/` 匹配，而 wecert 对**所有**它上传的证书都用这个前缀 —— 包括当前正在服务的那张。它会先打印清单并要求确认；非交互式的 stdin 一律按"否"处理。
