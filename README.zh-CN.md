...
| `-require-trusted` | `false` | 服务中的证书链无法通过系统根校验时判失败（守护进程侧 `probe.requireTrusted` 默认开；私有 CA 场景保持关闭） |
| `-wait` | `0` | 轮询到判定为 ok 或超时为止（例如 `90s`）—— 刚换完绑时有用 |
| `-json` | `false` | 每次尝试输出一个 JSON 对象（NDJSON：`-wait` 下的重试、或解析到多个地址的主机，都各占一行） |

退出码：`0` 与预期一致 · `1` 探测根本没跑成 · `2` 探测跑成了，但服务的是错的证书 · `64` 命令行本身写错了。请求帮助不是错误：`-h` 打印用法并以 `0` 退出。

`-json` 下 `verdict.problems` 是对象数组（`{"kind": "...", "text": "..."}`），每个独立问题一条 ——
因为排查方向不同：`min_valid_for` 表示服务的就是部署的那张证书、只是剩余有效期不够（续期还没跑），
`validity_window` 表示服务中的证书已过期或尚未生效，`untrusted` 表示证书链过不了系统根校验，
