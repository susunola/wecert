...
inventory. Filter by account or status, search domains and CLB IDs, and open any
row for deployment and TLS evidence. The console includes light and dark themes
and a mobile layout, with no frontend build or external assets.

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/console-dark.png">
  <img src="docs/console-overview.png" alt="Read-only certificate inventory console" width="1200">
</picture>

*Example data rendered by the production console.*

To enable it, add the following to your existing configuration, replace the token
with a secret generated by `openssl rand -hex 32`, and restart the daemon:
...
With the installed systemd service, run `sudo systemctl restart wecert`. The
console is **`GET /status`** and its JSON API is **`GET /api/inventory`** on that
listener. Both require the same Bearer token as the webhook; an ordinary browser
visit without an authentication header returns `401`.

For a quick view, set `WECERT_TOKEN` to your configured token and fetch an
interactive local snapshot:
...
Open `console.html` in your browser. To get new data, fetch it again. For live
browser access, use the [loopback proxy and SSH tunnel example](docs/console.md#live-browser-access),
or your existing authenticated TLS gateway. The console needs a running daemon;
