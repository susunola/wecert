...
// ClearRateBucketReset forgets a CA-reported deadline once it has passed, so the estimate is
// what governs again rather than a stale instant.
func (s *Store) ClearRateBucketReset(limitName, scopeID string, now time.Time) error {
	s.mu.Lock()
	defer s.mu.Unlock()

	_, err := s.db.Exec(`
		UPDATE rate_buckets SET reset_at = 0, reset_reason = ''
		WHERE limit_name = ? AND scope_id = ? AND reset_at != 0 AND reset_at <= ?`,
		limitName, scopeID, toUnix(now))
	if err != nil {
		return fmt.Errorf("clear rate bucket reset %s/%s: %w", limitName, scopeID, err)
	}
	return nil
}

// ListRateBuckets returns every recorded bucket, for diagnostics and metrics refresh.
func (s *Store) ListRateBuckets() ([]*RateBucket, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	rows, err := s.db.Query(`
		SELECT limit_name, scope_id, tokens, observed_at, reset_at, reset_reason
		FROM rate_buckets ORDER BY limit_name, scope_id`)
	if err != nil {
		return nil, fmt.Errorf("list rate buckets: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&b.LimitName, &b.ScopeID, &b.Tokens, &observedAt, &resetAt, &b.ResetReason); err != nil {
			return nil, fmt.Errorf("scan rate bucket: %w", err)
		}
		b.ObservedAt = fromUnix(observedAt)
		b.ResetAt = fromUnix(resetAt)
		out = append(out, &b)
	}
	return out, rows.Err()
}
