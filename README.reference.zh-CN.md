...
每个二进制内建四种 provider，覆盖了这个程序真正要服务的部署形态：**DNSPod**（自有 API Token）、
**腾讯云 DNS**（CAM 凭证，推荐，因为能和证书部署共用一套）、**Cloudflare**（受限 API Token）、
**Route 53**（AWS 凭证，实例角色即可）。四者的凭证体系互不相同，别搞混；另外还有 lego 注册表里
的约 198 个 provider，需要用 `-tags lego_dns` 自行编译。
...
如果发来的**每一个**名字都落在 `unknown` 里，返回的是 `404` 而不是 `202`：没有任何东西在收敛，`202` 会让发布钩子以为这次触发生效了。部分命中（有的名字已启动、有的不认识）仍然是 `202`，不认识的名字列在 `unknown` 里。
