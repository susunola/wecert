...
- **CA 侧"标识符暂停"的真实报文**（U21/U44，**本遍之后补记**）：**识别已实现、有会变红的单测**。第五个限额 `consecutive-authz-failures-per-identifier`（1,152 次连续授权失败、每个标识符每天回 1、成功即清零；越界后 CA 暂停该 (账号, 标识符) 并要求走自助门户解停）现在会被 `refusedLimits` 认出，两种措辞都覆盖：文档里的 `too many failed authorizations recently: …` 与 Boulder 现行的 "Your account is temporarily prevented from requesting certificates for …"。区分它和每小时失败预算的依据是**报文里有没有时刻**：预算模板必定带 `retry after`，暂停限额在 Boulder 里根本没有对应报文，所以"有 failed authorizations 字样但没有任何时刻"就是暂停。CA 不给时刻时按**发布速率记一天的下限**（一天 = 每标识符回 1 的间隔；是**下限不是测量**，因为只有 CA 自助门户能解除暂停），暂停期间不再下单、日志点名暂停并给出门户链接；发现未给时刻的普通拒绝（例如被截断的注册域报文）仍按原退避重试。**仍未验证的是 CA 的真实报文本身**：要诱发它必须**在 CA 查询的那一刻**让权威 DNS 给出失败答案 —— 本机做不到（DNSPod 权威删除传播 ~60s，够不上 CA 查询窗口），要么把一个子域委派给有公网 53 端口的机器（= 再开一台 CVM），要么在 Boulder 侧用固定装置。单测钉的是措辞解析、作用域与时限长度，不是 CA 的行为（同一行见 §7.3）。
...
| U21 / U44（连续授权失败导致的标识符暂停、真实失败下的配额行为） | **识别已实现、有会变红的单测**（第五个限额 `consecutive-authz-failures-per-identifier`：1,152 次连续失败 / 每天回 1 / 成功即清零；两种措辞都认，无时刻时记**一天的下限**，暂停期间不再下单；发现无时刻的普通拒绝仍照旧退避重试 —— 见 `docs/backlog.md` 2b 与 §4.2 末条）。**剩下的卡点只有"CA 的真实报文"本身**：需要**在 CA 查询的那一刻**让权威 DNS 给出失败答案。本机做不到（DNSPod 的权威删除传播要 ~60s，够不上 CA 查询窗口）；要做必须把一个子域委派给一台有公网 53 端口的机器 —— 也就是再开一台 CVM |
