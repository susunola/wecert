...
|---|---|---|---|
| **A. Faster recovery from backup** — snapshot to remote storage | Host loss, with a recovery window measured in minutes | `stateBackup.remoteTargets` sends coherent SQLite snapshots to S3, Tencent COS or SFTP; **restore is one command** (`wecert -restore remote:<target>`, which keeps the database it replaces and records the rate-limit caveat for the next start). `docs/recovery.md` is the procedure. | **Yes.** This is the honest answer to "the host died" today. The remote destination is an intentional operational dependency, not a second running wecert instance. |
...
A addresses the actual exposure: how long it takes to get certificates renewing again after a host
is lost. The snapshot format, retention, remote transports and restore command (`wecert -restore
remote:<target>`) are implemented. The remaining operator decision is where to keep the copy and
what recovery-time objective it buys; it is not a reason to put live SQLite on network storage.
