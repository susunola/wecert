...
> **状态：本节核实过的 15 条已全部修复**（B1、B2、M1–M7、m1–m4），每条都有"没有它测试就红"的用例；提交见 `67b9e44`、`9c117e4`、`30d09b1`、`da6da84`。未核实的 6 条仍未处理，见第 3 节。
...
- **修复状态**：核实的 15 条全部修完（见第 2 节各条的「已修」），未核实的 6 条（`reconcile.go` 的 `RunOnce` 丢弃 `RunReport`、`spec/observe.go` 的 frozen shadow、时钟回拨重复计息、以及 3 条 minor）**未处理**——它们仍是「工具报告、待确认」。
