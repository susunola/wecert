...
| N3 | `internal/acme/revoke.go`（`alreadyRevoked`） | ACME 的 `alreadyRevoked` 被当成可重试：`ClearRevokeRequest` 只在成功路径，于是请求行永不清理 → `wecert_revocation_pending` 恒 ≥1、CRITICAL 告警常亮、每轮重试，而 CLI 对**已吊销**的证书持续说「会重试」；代码注释还把这次重试称为「harmless」 | 识别 `urn:ietf:params:acme:error:alreadyRevoked`（`errors.As` 到 `*legoacme.ProblemDetails`，文本兜底）为终态并清行；`TestAlreadyRevokedIsTerminalAndClearsTheRequest` 同时钉住「普通失败仍可重试、行保留」（去掉终态分支即变红） |
