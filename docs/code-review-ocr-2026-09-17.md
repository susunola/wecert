...
| N3 | `cmd/wecert-probe/main.go` | `-wait` 只在两次尝试之间检查，**进行中的那次尝试会用满自己的 `-timeout`**（默认 10s）：实测 `-wait 1s` 用 10.009s、`-wait 1s -timeout 5s` 用 5.008s，与本文件「不得等超过要求的时长」的自我要求矛盾 | 每次尝试的 `Options.Timeout` 取「剩余预算」与自身超时的较小值，预算耗尽就返回上一次的判定；`TestWaitBoundsAnAttemptAlreadyInFlight` 用一个「阻塞到自己的超时才返回」的假探测器断言 120ms 预算不会跑成数秒（把钳制关掉即变红） |
| `internal/acme/manager_done.go`（部署路径的两处 `PutOrder`） | 写入失败直接 `return err` 而不走 `recordFailure`：没有退避（连为这种情形准备的进程内 `transientBackoff` 也没有），且刚上传的云证书 ID 丢失，下一轮重新 `Upload`，每轮泄漏一张云证书 | 两处都改为 `recordFailure` 并说清「已上传但记账失败」；`TestAFailedDeployBookkeepingSchedulesARetry` 用一个 `WHEN NEW.deployment_cert_id != OLD.deployment_cert_id` 的触发器只挡住锚点写入（其余订单更新照常），断言失败被记下且有退避，改回裸 `return err` 即变红 |
...
| `internal/state/revoke.go` + `internal/acme/revoke.go` | 持久化的吊销请求**不记录证书身份**：`revoke_requests` 只有 name/reason/时间/次数，重试时 `processRevocation` 读的是**当前** `certificates.cert_pem`，而续期会覆盖它；reconcile 又在每轮证书循环**之后**重试吊销——于是同一轮里可以先提升新证书再把它吊销：**新证书被吊销**、泄露的旧证书继续有效，而请求行被当作成功清掉（这是唯一一个错了无法挽回的动作） | 请求随证书身份落库：`revoke_requests.cert_identity`（`schemaColumns` 新增一列，老行为空 = 未知），值取 `certIdentity()`——即 ARI 的 certID（AKI + DER serial，RFC 9773 §4.1），本来就是本仓库「这是哪张证书」的答案，且只由证书自身推导；无 AKI 时退化为 serial，两种写法都由同一个函数产出，不会出现拼写不一致。重试时比对身份：相同 → 正常吊销；不同 → 从 `retired_certificates` 里找**被要求吊销的那张**的归档副本并吊销它（吊销请求由**账号密钥**签名，lego 的 `api.Core` 每个 JWS 都用账号 kid + 私钥，所以退役证书照样能吊）；归档也没有（保留期已回收）→ 报错、**不清行**、`wecert_revocation_pending` 继续告警，错误文案点名「需要在 CA 侧吊销」；名字下已无材料时同样回退到归档（这是 `retired_certificates` 第二个用途）。反之，显式再跑一次 `-revoke` 是**新的决定**：`AddRevokeRequest` 连同 reason 一起刷新身份（重试不经过这里），并在身份被改掉时打 warn。新增 `store.ListRetiredCertMaterial`。用例 5 条：重试跨过续期必须吊归档的旧证书（不是新的）、替换证书**永远不能**被顶替吊销且请求保持 outstanding、无身份的旧行仍按老行为吊销当前材料（不能因未知而卡死）、续期后再问一次要指向当前证书、无材料时从归档补吊；去掉身份比对 / 去掉归档回退 / 去掉身份刷新 三处变异各让对应用例变红 |
| `internal/reconcile/reconcile.go` | `publishQuota` 只有 `RunDetailed` 末尾一个调用者：webhook 触发的 pass 会花配额、也会记录 CA 的 Retry-After，但**从不发布**这两个指标；比轮询间隔（默认 1h）短的 deadline 更是永远来不及被发布成 blocked，critical 告警不会响 | 发布点移到 `startCert` 的 goroutine 里、`reconcileOne` **之后**：一处覆盖 `StartAll`/`StartNamed`/`StartCert` 全部 webhook 入口，而且是在花掉配额之后发布，数字里包含这一轮的花费（放在触发时发布就只是旧值）。用例 `TestAWebhookTriggeredPassPublishesQuotaWhenItFinishes` 用阻塞的假 manager 把 pass 卡在中间：在飞行中发布即变红，去掉发布也变红 |
| `internal/acme/manager.go` | ARI 守卫 `return ariErr` 绕过 `recordFailure`：导致失败的那次写正是携带 `ARICheckedAt` 的那次，而 `ARICheckedAt` 是**唯一**给 ARI 调用限速的东西——状态库坏掉时每一轮都会为每张证书重查 renewalInfo（正是 `renewalDecision` 的 `default:` 分支要避免的循环） | 改为 `return m.recordFailure(st, ariErr)`，把它无法落库的 backoff 补上；`recordFailure` 自带「取消的 pass 不算失败」过滤，所以停机不会把证书锁出下一个窗口。两条用例分别钉住两半：`TestAFailedARIDecisionSchedulesItsOwnBackoff`（在假 ARI 回调里关掉状态库，去掉 recordFailure 即变红）与 `TestACancelledPassDoesNotSetABackoff`（把取消也当失败记录即变红） |
| `internal/acme/manager_renew.go` | 新订单配额记在**第一次** `NewOrder` 上：真正创建订单的 `replaces` 重试不记账（下界乐观 1，而这正是「CA 不认 replaces」时批量下单的那条路），重试的限流拒绝也被静默丢弃（`NoteRetryAfter` 没跑） | 记账改到**决定结果的那次调用**上（抽出 `spendNewOrder`/`noteNewOrderRefusal`）：第一次失败不记账、重试成功记一次；两次失败各自解读（限流 deadline 来自哪次都算证据）。用例：重试造单必须恰好花 1（改成只在第一次记账即变红、改成两次都记也变红）、重试被限流也必须落下 deadline |
| `internal/onboarding/onboard.go` | `Commit` 先写报告、再写文档与状态：失败的那一轮会留下一个声称 `mode: "written"`、带一份**从未写出的 revision** 的报告，而报告正是人读的产物 | 报告改为**最后**写（抽出 `writeReport`，frozen 仍只写报告）：报告存在 ⟺ 这一轮完整落盘。`TestAFailedCommitWritesNoReport` 把文档目录设为不可写、报告目录可写，确认失败轮不留报告、且正常轮仍然留（只删不写也变红） |
| `internal/acme/dns.go` + `manager_flow.go` | `WaitAll` 注册的 TXT 租约可能**活得比它的记录久**（探针证明记录已不存在的未呈现行被直接删行；令牌刷新后 `CleanUp` 推导出的值也变了），于是该名字后续所有清理都走「还有别的挑战在线」分支，provider 的 delete-all 在进程重启前再也不会执行，TXT 记录滞留在那里占额度 | 两条丢失路径各自释放租约（`releaseStaleLease`）：令牌刷新时释放旧 token 那个值、探针证明不存在时释放被证明不存在的那个值。释放前查 `ListPresentedAuthorizations`，只要还有一个 presented 行声称「同名同值」就保留——那是唯一一种「本行不要了但别的证书还要」的情形，而保留只是推迟一次 delete-all，误删却会打掉另一张证书正等验证的记录（计费的一次 identifier 失败）。用例 3 条：探针证明不存在必须释放、令牌刷新必须释放旧值且仍要写新记录、另一个 presented 行仍声称该值时**不得**释放（三处各去掉一处释放/去掉保护即变红） |
| `internal/reconcile/reconcile.go` | `publishOrphans` 只是**窥探** claim（`isClaimable` 拿到答案就放锁）再在 store 读之后调 `CleanupOrphan`：webhook 触发的同步 claim 可以插进这个缝里（文档在两处 resolve 之间被改回来即可达），于是它的 TXT 记录/订单被拆掉——正是该处注释声称要防的破坏性交错 | 整个拆除期间**持有** claim（`acquire`），拆成 `tearDownOrphan` 并在其中 `defer release`，panic 也不会把名字卡死；`isClaimable` 随之删除（不再有调用者）。`TestTheOrphanTeardownHoldsTheClaimWhileItRuns` 用阻塞的假 manager 把拆除卡在中间，断言此时 claim 拿不到、结束后又拿得到（改回窥探、或去掉 release，即变红） |
...
| `internal/onboarding/onboard.go`（`applyGrace`） | 仍然被声明、但被 guard 1（或白名单）拒掉的名字，在同一轮就被**从期望状态里删除**：`stillDeclared` 分支只 `MarkPresent`，既不 carry 也不 `MarkAbsent`。guard 是网络读取的第二来源且**无法表达「结果可能不完整」**，一次规则抖动（区域缺失、部分可见、限流）就会静默剥掉线上覆盖；而 fuse 比较的是声明、声明没变，所以任何保护都不会触发。实测一次抖动 = 两次签发 + 两轮失去覆盖 | **语义反转（你拍板：按 carry 改）**：`stillDeclared` 的名字一律 **carry**——文档不变 → revision 不变 → 不触发签发，规则恢复时也不会再签一次；删掉的是「筛选器能摘覆盖」这条隐含规则。新规则一句话：**声明才是覆盖的依据**；守卫/白名单不通过只挡**新增**覆盖（从未被覆盖的名字照样被拒，`TestADeclarationWithNoRuleIsStillNotIssued` 钉住），要摘覆盖就删声明，之后走正常的宽限期 + 引用检查。报告里每个 hostname 只留一个结论（`unreject` 掉筛选器先写的那条排除决定），carry 的原因直接引用筛选器自己的话。用例：`TestAGuardWobbleDoesNotStripCoverageOfADeclaredName`（第 2 轮规则消失 → 名字仍在文档、revision 不变、恰好一条 included 决定；第 3 轮规则恢复 → revision 仍不变）、`TestADeclarationWithNoRuleIsStillNotIssued`、`TestATruncatedGuardAnswerIsReportedAndRemovesNothing`；去掉 carry / 去掉 guard 1 / 去掉 unreject 三处变异各自变红。文档同步：`docs/desired-state.md` §4/§6/§7 |
| `internal/onboarding/tencent.go` | 「结果可能不完整」无法表达：`listLoadBalancers` 只在 `len(list) < clbPageSize` 时收尾、从不比对 `TotalCount`；`listRuleDomainsFor` 直接信任一次 `DescribeListeners`（请求本身没有分页参数，应答里的 `TotalCount` 也没人看）；nil/无 id 的元素被**跳过**。于是「被截断的规则列表」和「完整但没这条规则」长得一模一样，而后者正是把已服务的名字判成无人引用、走进删除路径的形状 | 两处都按应答自带的总数校验：LB 累加 `read` 与 `TotalCount` 比对（少一个就在错误里点名 region 与两个数字），`DescribeListeners` 的 `len(Listeners)` 必须等于 `TotalCount`，无 id 的元素不再静默跳过而是判为不完整；统一成哨兵错误 `errIncompleteRuleList`。round 层把它当**独立信号**：`guardUnavailable` + 新增 `Report.GuardIncomplete`（JSON `guardIncomplete`），CLI 摘要单独一行说明「规则列表被截断了，不是重试就能好的天气」。用例：`TestListRuleDomainsRefusesATruncatedAnswer`（LB / listeners 两个子例）、`TestEnumerationSkipsNilListElements`（LB 子例按新契约改写：不解引用、也不静默跳过）、`TestATruncatedGuardAnswerIsReportedAndRemovesNothing`；去掉任一总数校验 / 把 nil 元素改回跳过 / 把不完整折进普通错误分支，各自变红 |
...
第二轮的 19 条发现全部核实并修复（见 §2.5），每一条都带一个「去掉修复就变红」的用例。最后一条（guard 1 抖动是否该摘掉覆盖）是一次语义反转，按你的决定改成了 carry，见 §2.5 末两行。

剩下的**同形状但未做**的地方，如实列在这里，不算已修：

- `internal/onboarding/tencent.go` 的**声明来源**（`listZones` / `listTXTRecords`）同样没有比对 DNSPod 应答里的总数，nil 元素也是跳过。它比守卫更靠近删除路径（声明少了 → 名字看起来没声明 → 宽限期后删除），但 DNSPod 的计数语义（是否受 Keyword/过滤影响）我**没有核实**，按错误假设加校验会让正常轮次误冻结，所以留作独立事项。
- `internal/acme/dns.go` 的 `CleanUp` 若在 `challengeLeases` 释放前进程崩溃，注册表内容随进程消失，没有跨进程的租约对账；这是设计上的（注册表是进程内的），重启后由 `cleanupOrphanTXT` 回收。
