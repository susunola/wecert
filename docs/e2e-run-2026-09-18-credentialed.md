...
- 人一旦把**旧**证书绑上去，下一轮就会自动切换；如果把**新**证书绑上去，既有的修复分支（`new 有绑定 / old 无绑定 → 认定切换已完成`）也会收敛。

但有一个语义问题值得你拍板：**"上传了但从未绑定"的续期，应该算失败吗？** 现在的行为是失败（每一轮都失败，直到有人绑定其中一个）。两种读法：

| 读法 | 代价 |
|---|---|
| 保持现状（失败）：配置要求部署，而部署确实没发生 —— 失败是诚实的；`consecutive_failures` 与日志都在喊"有人没做那一步" | 一个"还没人用"的证书会长期报失败；如果这个账号/证书本来就不打算绑（例如先上传、以后再绑），日志噪声会持续 |
| 改成"仍等首次绑定"（我倾向）：当 `deploy_confirmed=false` 且旧证书**可证明无绑定**时，不走 `UpdateCertificateInstance`，而是像首签那样上传新证书、记录新 id、重新打印"bind it once"提示 | 需要一次"旧证书有没有绑定"的枚举（已有 `bindingsWith`，但要接受"枚举不完整"这种答案）；并且"上传成功"与"部署成功"必须在状态里继续分得清楚（`deploy_confirmed` 已经是这个开关） |

我没有动它 —— 这是和守卫 carry 同一类的语义反转。

### 5.4 DNSPod 传播时延（数据点，不是缺陷）

本轮 5 次真实签发的传播等待：86s（L1）、约 2.5min（wildcard）、约 2min（tlsserver）、约 3.5min（Stage B 首次）、约 2.3min（CVM）。删除的传播到所有权威最长观察到 60s。这解释了 09-17 报告 5.1 的"判据说好了、CA 说 NXDOMAIN"为什么会发生，也说明**等待预算不能按最好情况设定**。
