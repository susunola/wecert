...
- **驳回与未验证分开写**：§1.2 是"我实测它成立"，§1.3 是"我没跑"，两者都不是"没问题"。
- **这一轮的未验证项随后被专门核查过**：本报告写完之后又跑了一轮实测核查（[verification-2026-09-18.md](verification-2026-09-18.md)），把上面这些条目逐条真跑了 —— 真机**生产** LE 签发与 ARI 驱动续期、真实 DNSPod 记录名形态、真实端点探针三态、容器里 `install.sh` 全流程与 145 次 SIGKILL、完整恢复演练、老二进制读新 schema、真实 CLB 的 `IsCheckResource` 语义、`docs/certificate-lifecycle*.html` 的 63 条断言 —— **又找出并修掉 10 个缺陷**（含本轮自己引入的一个 `-clb` 回归）。仍然做不到的 13 条与本质上无法验证的 7 条在那份文件里逐条写了卡点；"真实断电/掉页"用系统调用级 fsync/rename 故障注入覆盖了其中"持久化调用失败"的一半，另一半（扇区级损坏、页缓存丢失）仍然无法在这里制造。
